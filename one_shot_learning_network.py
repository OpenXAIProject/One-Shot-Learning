...
class Classifier:
    def __init__(self, batch_size, layer_sizes, num_channels=1):
        ...
        self.weights = {
            'h1': tf.Variable(initial_value=initializer([93, self.layer_sizes[0]], dtype=tf.float32), trainable=True),
            'h2': tf.Variable(initial_value=initializer([self.layer_sizes[0], self.layer_sizes[1]], dtype=tf.float32), trainable=True),
            'h3': tf.Variable(initial_value=initializer([self.layer_sizes[1], self.layer_sizes[2]], dtype=tf.float32), trainable=True),
            'h4': tf.Variable(initial_value=initializer([self.layer_sizes[2], self.layer_sizes[3]], dtype=tf.float32), trainable=True)
            # 'out': tf.Variable(tf.random_normal([self.layer_sizes[0], 2]))
        }
        self.biases = {
            'b1': tf.Variable(initial_value=tf.constant(0.1, shape=[self.layer_sizes[0]], dtype=tf.float32), trainable=True),
            'b2': tf.Variable(initial_value=tf.constant(0.1, shape=[self.layer_sizes[1]], dtype=tf.float32), trainable=True),
            'b3': tf.Variable(initial_value=tf.constant(0.1, shape=[self.layer_sizes[2]], dtype=tf.float32), trainable=True),
            'b4': tf.Variable(initial_value=tf.constant(0.1, shape=[self.layer_sizes[3]], dtype=tf.float32), trainable=True)
    ...
    def __call__(self, image_input, training=False, keep_prob=1.0):
        ...
        with tf.variable_scope('g', reuse=self.reuse):
            ...
            with tf.variable_scope('conv_layers'):
                with tf.variable_scope('g_mlp1'):
                    g_mlp1_encoder = tf.add(tf.matmul(image_input, self.weights['h1']), self.biases['b1'])
                    g_mlp1_encoder = relu(g_mlp1_encoder, name='outputs')
                    g_mlp1_encoder = tf.contrib.layers.batch_norm(g_mlp1_encoder, updates_collections=None, decay=0.99,
                                                                  scale=True, center=True, is_training=training)
                    # g_mlp1_encoder = tf.nn.dropout(g_mlp1_encoder, keep_prob=keep_prob)

                with tf.variable_scope('g_mlp2'):
                    g_mlp2_encoder = tf.add(tf.matmul(g_mlp1_encoder, self.weights['h2']), self.biases['b2'])
                    g_mlp2_encoder = relu(g_mlp2_encoder, name='outputs')
                    g_mlp2_encoder = tf.contrib.layers.batch_norm(g_mlp2_encoder, updates_collections=None, decay=0.99,
                                                scale=True, center=True, is_training=training)
                    # g_mlp2_encoder = tf.nn.dropout(g_mlp2_encoder, keep_prob=keep_prob)
                # with tf.variable_scope('g_mlp3'):
                #     g_mlp3_encoder = tf.add(tf.matmul(g_mlp2_encoder, self.weights['h3']), self.biases['b3'])
                #     g_mlp3_encoder = tanh(g_mlp3_encoder, name='outputs')
                #     g_mlp3_encoder = tf.contrib.layers.batch_norm(g_mlp3_encoder, updates_collections=None, decay=0.99,
                #                                                   scale=True, center=True, is_training=training)
                #     # g_mlp1_encoder = tf.nn.dropout(g_mlp1_encoder, keep_prob=keep_prob)
                #
                # with tf.variable_scope('g_mlp4'):
                #     g_mlp4_encoder = tf.add(tf.matmul(g_mlp3_encoder, self.weights['h4']), self.biases['b4'])
                #     g_mlp4_encoder = tanh(g_mlp4_encoder, name='outputs')
                #     g_mlp4_encoder = tf.contrib.layers.batch_norm(g_mlp4_encoder, updates_collections=None, decay=0.99,
                #                                                   scale=True, center=True, is_training=training)
                    # g_mlp2_encoder = tf.nn.dropout(g_mlp2_encoder, keep_prob=keep_prob)
                # with tf.variable_scope('g_conv1'):
                #     g_conv1_encoder = tf.layers.conv2d(image_input, self.layer_sizes[0], [3, 1], strides=(1, 1),
                #                                        padding='SAME')
                ...
                #                                                    scale=True, center=True, is_training=training)
                #     g_conv1_encoder = max_pool(g_conv1_encoder, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1],
                #                                padding='SAME')
                #     g_conv1_encoder = tf.nn.dropout(g_conv1_encoder, keep_prob=keep_prob)
                #
                # with tf.variable_scope('g_conv2'):
                #     g_conv2_encoder = tf.layers.conv2d(g_conv1_encoder, self.layer_sizes[1], [3, 1], strides=(1, 1),
                #                                        padding='SAME')
                ...
                #                                                    decay=0.99,
                #                                                    scale=True, center=True, is_training=training)
                #     g_conv2_encoder = max_pool(g_conv2_encoder, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1],
                #                                padding='SAME')
                #     g_conv2_encoder = tf.nn.dropout(g_conv2_encoder, keep_prob=keep_prob)
                #
                # with tf.variable_scope('g_conv3'):
                #     g_conv3_encoder = tf.layers.conv2d(g_conv2_encoder, self.layer_sizes[2], [3, 1], strides=(1, 1),
                #                                        padding='SAME')
                ...
                #                                                    decay=0.99,
                #                                                    scale=True, center=True, is_training=training)
                #     g_conv3_encoder = max_pool(g_conv3_encoder, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1],
                #                                padding='SAME')
                #     g_conv3_encoder = tf.nn.dropout(g_conv3_encoder, keep_prob=keep_prob)
                #
                # with tf.variable_scope('g_conv4'):
                #     g_conv4_encoder = tf.layers.conv2d(g_conv3_encoder, self.layer_sizes[1], [3, 1], strides=(1, 1),
                #                                        padding='SAME')
                ...
                #                                                    decay=0.99,
                #                                                    scale=True, center=True, is_training=training)
                #     g_conv4_encoder = max_pool(g_conv4_encoder, ksize=[1, 2, 1, 1], strides=[1, 2, 1, 1],
                #                                padding='SAME')
                #     g_conv4_encoder = tf.nn.dropout(g_conv4_encoder, keep_prob=keep_prob)

            # g_conv_encoder = g_mlp4_encoder
            g_conv_encoder = tf.contrib.layers.flatten(g_mlp2_encoder)
...
class MatchingNetwork:
    def __init__(self, support_set_images, support_set_labels, target_image, target_label, keep_prob,
        ...
        self.batch_size = batch_size
        self.fce = fce
        self.g = Classifier(self.batch_size, num_channels=num_channels, layer_sizes=[64, 64, 64, 64])#g:[batch_size,64]
        # self.g = trainFeatures
        if fce:
            self.lstm = BidirectionalLSTM(layer_sizes=[93], batch_size=self.batch_size)
        ...
        # self.target_image = targetFeature
        ...
        # similarity = self.dn(support_set=trainFeatures, input_image=targetFeature, name='AD')
        # pred = self.classify(similarity, self.support_set_labels, 'AD')
    def loss(self):
        """
        ...
        with tf.name_scope("losses"):
            ...
            encoded_images = []
            [b, num_classes, spc, h] = self.support_set_images.get_shape().as_list()
            self.support_set_images = tf.reshape(self.support_set_images, shape=(b,  num_classes*spc, h))
