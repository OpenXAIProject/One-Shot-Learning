...
class ADNIDataset():
    def __init__(self, k, batch_size, classes_per_set=10, samples_per_class=1, seed=2591, shuffle_classes=False):
        ...
        :param classes_per_set: Integer indicating the number of classes per set
        :param samples_per_class: Integer indicating samples per class
        :Data - AD: 198*93, CN: 229*93, MCI: 374*93
        e.g. For a 20-way, 1-shot learning task, use classes_per_set=20 and samples_per_class=1
             For a 5-way, 10-shot learning task, use classes_per_set=5 and samples_per_class=10
        """
        #
        # def k_fold_cross_validation(items, k=10, randomize=False):
        ...
        #     return slices
        np.random.seed(seed)
        # data = sio.loadmat("Mean_ROI_Features.mat")
        data = sio.loadmat("ADNI_%dfold.mat" % k)
        print("ADNI_%dfold.mat" % k)
        # ad_feature = np.transpose(data['MCI_Feature']).reshape((1, 374, 93))
        # ad_feature = np.transpose(data['AD_Feature']).reshape((1, 198, 93))
        # nc_feature = np.transpose(data['NC_Feature']).reshape((1, 229, 93))
        # ad_feature = np.transpose(data['AD_Feature']).reshape([1, 198, 93])
        # nc_feature = np.transpose(data['NC_Feature'][:, :-31]).reshape([1, 198, 93])

        # ad_k_fold = k_fold_cross_validation(ad_feature, 10)
        # np.save('./ADNI_AD_data.npy', ad_k_fold)
        # nc_k_fold = k_fold_cross_validation(nc_feature, 10)
        # np.save('./ADNI_NC_dat
        # 100%
        ...
        # 50%
        # self.x_train = data['train'][:, 0:int(round(data['train'].shape[1]/2)), :]
        # self.x_test = data['test'][:, 0:int(round(data['train'].shape[1]/2)), :]
        # self.x_val = data['val'][:, 0:int(round(data['train'].shape[1]/2)), :]
        # self.x_train = np.concatenate((ad_feature[:, :-108, :], nc_feature[:, :-139, :]), 0)
        # self.x_test = np.concatenate((ad_feature[:, -18:-9, :], nc_feature[:, -49:-40, :]), 0)
        # self.x_val = np.concatenate((ad_feature[:, -9:, :], nc_feature[:, -9:, :]), 0)

        # 30%
        # self.x_train = data['train'][:, 0:int(round(data['train'].shape[1]/3)), :]
        # self.x_test = data['test'][:, 0:int(round(data['train'].shape[1]/3)), :]
        # self.x_val = data['val'][:, 0:int(round(data['train'].shape[1]/3)), :]
        # self.x_train = np.concatenate((ad_feature[:, :-144, :], nc_feature[:, :-175, :]), 0)
        # self.x_test = np.concatenate((ad_feature[:, -18:-9, :], nc_feature[:, -49:-40, :]), 0)
        # self.x_val = np.concatenate((ad_feature[:, -9:, :], nc_feature[:, -9:, :]), 0)

        # 10%
        # self.x_train = data['train'][:, 0:int(round(data['train'].shape[1]/10)), :]
        # self.x_test = data['test'][:, 0:int(round(data['train'].shape[1]/10)), :]
        # self.x_val = data['val'][:, 0:int(round(data['train'].shape[1]/10)), :]

        # self.x_train, self.x_test, self.x_val = self.x[:1200], self.x[1200:1411], self.x[1411:]
        self.mean = np.mean(np.concatenate((self.x_train, self.x_val), 1))
        self.std = np.std(np.concatenate((self.x_train, self.x_val), 1))
        self.batch_size = batch_size
        self.n_classes = 2
        self.classes_per_set = classes_per_set
        self.samples_per_class = samples_per_class
    ...
    def sample_new_batch(self, data_pack):
        ...
        support_set_x = np.zeros((self.batch_size, self.classes_per_set, self.samples_per_class, data_pack.shape[2]),
                                   dtype=np.float32)
        support_set_y = np.zeros((self.batch_size, self.classes_per_set, self.samples_per_class), dtype=np.float32)
        target_x = np.zeros((self.batch_size, data_pack.shape[2]), dtype=np.float32)
        target_y = np.zeros((self.batch_size,), dtype=np.float32)
        for i in range(self.batch_size):
            classes_idx = np.arange(data_pack.shape[0])
            samples_idx = np.arange(data_pack.shape[1])
            choose_classes = np.random.choice(classes_idx, size=self.classes_per_set, replace=True)
            choose_label = np.random.choice(self.classes_per_set, size=1)
            choose_samples = np.random.choice(samples_idx, size=self.samples_per_class+1, replace=True)
